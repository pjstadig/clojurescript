...
(ns cljel.repl
  ...
  (:require [clojure.string :as string]
            ...
            [cljel.compiler :as comp]
            [cljel.analyzer :as ana]
            [cljel.tagged-literals :as tags]
            [cljel.closure :as cljelc]))

(def ^:dynamic *cljel-verbose* false)
...
(defn load-namespace
  ...
  (let [sym (if (and (seq? sym)
        ...
        deps (->> (cljelc/add-dependencies opts {:requires [(name sym)] :type :seed})
  ...
  ([repl-env env filename form wrap]
     (try
       (let [ast (ana/analyze env form)
             ...
             wrap-js (comp/emit-str (binding [ana/*cljel-warn-on-undeclared* false
                                              ana/*cljel-warn-on-redef* false
                                              ana/*cljel-warn-on-dynamic* false
                                              ana/*cljel-warn-on-fn-var* false
                                              ana/*cljel-warn-fn-arity* false]
  ...
  (with-open [r (io/reader stream)]
    (let [env (ana/empty-env)
      ...
      (loop [r (read pbr false eof false)]
        (let [env (assoc env :ns (ana/get-namespace ana/*cljel-ns*))]
...
(defn- wrap-fn [form]
  (cond (and (seq? form) (= 'ns (first form))) identity
        ('#{*1 *2 *3} form) (fn [x] `(cljel.core.pr-str ~x))
        :else (fn [x] `(cljel.core.pr-str
                       (let [ret# ~x]
                         (do (set! *3 *2)
                             (set! *2 *1)
                             (set! *1 ret#)
...
(defn- eval-and-print [repl-env env form]
  (let [ret (evaluate-form repl-env
                           (assoc env :ns (ana/get-namespace ana/*cljel-ns*))
                           "<cljel repl>"
                           form
                           (wrap-fn form))]
...
(defn- read-next-form []
  (try {:status :success :form (binding [*ns* (create-ns ana/*cljel-ns*)
  ...
  (let [load-file-fn (fn [repl-env file] (load-file repl-env file))]
    {'in-ns (fn [_ quoted-ns]
              (let [ns-name (second quoted-ns)]
                ...
                (set! ana/*cljel-ns* ns-name)))
     'load-file load-file-fn
     'clojure.core/load-file load-file-fn
     'load-namespace (fn [repl-env ns] (load-namespace repl-env ns))}))

(defn analyze-source
  "Given a source directory, analyzes all .cljel files. Used to populate
  cljel.analyzer/namespaces so as to support code reflection."
  [src-dir]
  (if-let [src-dir (and (not (empty? src-dir))
                     (File. src-dir))]
    (doseq [file (comp/cljel-files-in src-dir)]
      (ana/analyze-file (str "file://" (.getAbsolutePath file))))))

(defn repl
  "Note - repl will reload core.cljel every time, even if supplied old repl-env"
  [repl-env & {:keys [analyze-path verbose warn-on-undeclared special-fns]}]
  (prn "Type: " :cljel/quit " to quit")
  (binding [ana/*cljel-ns* 'cljel.user
            *cljel-verbose* verbose
            ana/*cljel-warn-on-undeclared* warn-on-undeclared]
    (when analyze-path
    ...
    (let [env {:context :expr :locals {}}
      ...
      (-setup repl-env)
      (loop []
        (print (str "ClojureScript:" ana/*cljel-ns* "> "))
        (flush)
        (let [{:keys [status form]} (read-next-form)]
          (cond
           (= form :cljel/quit) :quit
