...
(def fast-path-protocols
  ...
  (zipmap (map #(symbol "cljel.core" (core/str %))
...
(defmacro extend-type [tsym & impls]
  (let [resolve #(let [ret (:name (cljel.analyzer/resolve-var (dissoc &env :locals) %))]
  ...
  (let [r (:name (cljel.analyzer/resolve-var (dissoc &env :locals) t))
    ...
    (if (seq impls)
      `(do
         ...
         (set! (.-cljel$lang$type ~t) true)
         (set! (.-cljel$lang$ctorPrSeq ~t) (fn [this#] (list ~(core/str r))))
         (set! (.-cljel$lang$ctorPrWriter ~t) (fn [this# writer# opt#] (-write writer# ~(core/str r))))
      ...
      `(do
         ...
         (set! (.-cljel$lang$type ~t) true)
         (set! (.-cljel$lang$ctorPrSeq ~t) (fn [this#] (list ~(core/str r))))
         (set! (.-cljel$lang$ctorPrWriter ~t) (fn [this# writer# opts#] (-write writer# ~(core/str r))))
         ~t))))
...
(defmacro defrecord [rsym fields & impls]
  (let [r (:name (cljel.analyzer/resolve-var (dissoc &env :locals) rsym))]
    `(let []
       ~(emit-defrecord &env rsym r fields impls)
       (set! (.-cljel$lang$type ~r) true)
       (set! (.-cljel$lang$ctorPrSeq ~r) (fn [this#] (list ~(core/str r))))
       (set! (.-cljel$lang$ctorPrWriter ~r) (fn [this# writer#] (-write writer# ~(core/str r))))
       ~(build-positional-factory rsym r fields)
       ~(build-map-factory rsym r fields)
       ~r)))

(defmacro defprotocol [psym & doc+methods]
  (let [p (:name (cljel.analyzer/resolve-var (dissoc &env :locals) psym))
...
(defmacro satisfies?
  ...
  (let [p (:name (cljel.analyzer/resolve-var (dissoc &env :locals) psym))
        ...
        msym (symbol (core/str "-cljel$lang$protocol_mask$partition" part "$"))]
    `(let [~xsym ~x]
       (if ~xsym
         (if (or ~(if bit `(unsafe-bit-and (. ~xsym ~msym) ~bit))
           ...
           (if (coercive-not (. ~xsym ~msym))
             (cljel.core/type_satisfies_ ~psym ~xsym)
             false))
         (cljel.core/type_satisfies_ ~psym ~xsym)))))

(defmacro lazy-seq [& body]
  `(new cljel.core/LazySeq nil false (fn [] ~@body) nil))
...
(defmacro delay [& body]
  ...
  calls."
  `(new cljel.core/Delay (atom {:done false, :value nil}) (fn [] ~@body)))
...
(defmacro assert
  ...
  ([x]
     (when *assert*
       `(when-not ~x
          (throw (js/Error.
                  (cljel.core/str "Assert failed: " (cljel.core/pr-str '~x)))))))
  ([x message]
     (when *assert*
       `(when-not ~x
          (throw (js/Error.
                  (cljel.core/str "Assert failed: " ~message "\n" (cljel.core/pr-str '~x))))))))
...
(defmacro defmethod
  ...
  `(-add-method ~(with-meta multifn {:tag 'cljel.core/MultiFn}) ~dispatch-val (fn ~@fn-tail)))
...
(defn gen-apply-to-helper
  ...
  ([n]
     (let [prop (symbol (core/str "-cljel$lang$arity$" n))
           f (symbol (core/str "cljel$lang$arity$" n))]
       (if (core/<= n 20)
