...
(declare warning)
(def ^:dynamic *cljel-warn-on-undeclared* false)
(declare confirm-bindings)
(declare ^:dynamic *cljel-file*)
...
(defonce namespaces (atom '{cljel.core {:name cljel.core}
                            cljel.user {:name cljel.user}}))

(defn reset-namespaces! []
  (reset! namespaces
    '{cljel.core {:name cljel.core}
      cljel.user {:name cljel.user}}))
...
(def ^:dynamic *cljel-ns* 'cljel.user)
(def ^:dynamic *cljel-file* nil)
(def ^:dynamic *cljel-warn-on-redef* true)
(def ^:dynamic *cljel-warn-on-dynamic* true)
(def ^:dynamic *cljel-warn-on-fn-var* true)
(def ^:dynamic *cljel-warn-fn-arity* true)
(def ^:dynamic *cljel-warn-fn-deprecated* true)
(def ^:dynamic *cljel-warn-protocol-deprecated* true)
(def ^:dynamic *unchecked-if* (atom false))
(def ^:dynamic *cljel-static-fns* false)
(def ^:dynamic *cljel-macros-path* "/cljel/core")
(def ^:dynamic *cljel-macros-is-classpath* true)
(def  -cljel-macros-loaded (atom false))

(defmacro no-warn [& body]
  `(binding [*cljel-warn-on-undeclared* false
             *cljel-warn-on-redef* false
             *cljel-warn-on-dynamic* false
             *cljel-warn-on-fn-var* false
             *cljel-warn-fn-arity* false
             *cljel-warn-fn-deprecated* false]
     ~@body))

(defn load-core []
  (when (not @-cljel-macros-loaded)
    (reset! -cljel-macros-loaded true)
    (if *cljel-macros-is-classpath*
      (load *cljel-macros-path*)
      (load-file *cljel-macros-path*))))

(defmacro with-core-macros
  [path & body]
  `(do
     (when (not= *cljel-macros-path* ~path)
       (reset! -cljel-macros-loaded false))
     (binding [*cljel-macros-path* ~path]
       ~@body)))
...
(defmacro with-core-macros-file
  ...
  `(do
     (when (not= *cljel-macros-path* ~path)
       (reset! -cljel-macros-loaded false))
     (binding [*cljel-macros-path* ~path
               *cljel-macros-is-classpath* false]
       ~@body)))

(defn empty-env []
  {:ns (@namespaces *cljel-ns*) :context :statement :locals {}})
...
(defn message [env s]
  (str s (when (:line env)
           (str " at line " (:line env) " " *cljel-file*))))
...
(defn error
  ...
  ([env s cause]
   (ex-info (message env s)
            (assoc (source-info env) :tag :cljel/analysis-error)
            cause)))

(defn analysis-error? [ex]
  (= :cljel/analysis-error (:tag (ex-data ex))))
...
(defn core-name?
  ...
  [env sym]
  (and (get (:defs (@namespaces 'cljel.core)) sym)
       ...
       (let [s (str sym)
         ...
         (cond
           ...
           (let [ns (namespace sym)
                 ns (if (= "clojure.core" ns) "cljel.core" ns)
...
(defn confirm-bindings [env names]
  (doseq [name names]
    (let [env (merge env {:ns (@namespaces *cljel-ns*)})
          ev (resolve-existing-var env name)]
      (when (and *cljel-warn-on-dynamic*
                 ev (not (-> ev :dynamic)))
        (warning env
...
(defmethod parse 'def
  ...
  (let [pfn (fn
    ...
    (let [env (if (or (and (not= ns-name 'cljel.core)
                           (core-name? env sym))
                      (get-in @namespaces [ns-name :uses sym]))
                (let [ev (resolve-existing-var (dissoc env :locals) sym)]
                  (when *cljel-warn-on-redef*
                    (warning env
...
(defn ns->relpath [s]
  (str (string/replace (munge-path s) \. \/) ".cljel"))
...
(defn parse-invoke
  ...
  (disallowing-recur
   (let [enve (assoc env :context :expr)
         ...
         argexprs (vec (map #(analyze enve %) args))
         argc (count args)]
     (if (and *cljel-warn-fn-arity* (-> fexpr :info :fn-var))
     ...
     (if (and *cljel-warn-fn-deprecated* (-> fexpr :info :deprecated)
  ...
  (let [mvar
        (when-not (or (-> env :locals sym)        ;locals hide macros
          ...
          (if-let [nstr (namespace sym)]
            (when-let [ns (cond
                           (= "clojure.core" nstr) (find-ns 'cljel.core)
            ...
            (if-let [nsym (-> env :ns :uses-macros sym)]
              ...
              (.findInternedVar ^clojure.lang.Namespace (find-ns 'cljel.core) sym))))]
...
(defn analyze-file
  ...
  (let [res (if (re-find #"^file://" f) (java.net.URL. f) (io/resource f))]
    ...
    (binding [*cljel-ns* 'cljel.user
              *cljel-file* (.getPath ^java.net.URL res)
              *ns* *reader-ns*]
      (with-open [r (io/reader res)]
        (let [env (empty-env)
          ...
          (loop [r (read pbr false eof false)]
            (let [env (assoc env :ns (get-namespace *cljel-ns*))]
